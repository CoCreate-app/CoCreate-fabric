...
function initInputs() {
	let inputs = document.querySelectorAll("[fabric-query]");
	for (let input of inputs) {
		let canvasElements = queryElements({
			element: input,
			prefix: "fabric"
		});
		...
		if (input.type === "file") {
			...
			input.addEventListener(
				input.tagName.toLowerCase() === "input" ? "input" : "change",
...
function updateCanvas(element, canvasElements) {
	...
	for (let i = 0; i < element.length; i++) {
		if (!canvasElements || !canvasElements.length) {
			if (element[i].hasAttribute("print-query")) {
				canvasElements = queryElements({
					element: element[i],
					prefix: "fabric"
				});
			}
		...
		for (let j = 0; j < fabricAttr.length; j++) {
			if (j === 0) {
				fabricObject =
					fabricAttr[0].charAt(0).toUpperCase() +
					fabricAttr[0].slice(1);
			} else if (j === 1) {
				if (fabricAttr[1] !== "filters")
					fabricMethod =
						fabricAttr[1].charAt(0).toUpperCase() +
						fabricAttr[1].slice(1);
				else fabricMethod = fabricAttr[1];
			} else if (j === 2) {
				fabricProperty =
					fabricAttr[2].charAt(0).toUpperCase() +
					fabricAttr[2].slice(1);
			} else if (j === 3) {
...
function applyFilter(activeObject, fabricProperty, fabricParam, value) {
	...
	if (!fabricParam) fabricParam = fabricProperty.toLowerCase();

	let filterIndex = activeObject.filters.findIndex(
		(f) => f.type === fabricProperty
	);
...
function applyObjectProperty(
	...
	if (
		...
		typeof activeObject[fabricMethod] === "function"
	) {
		activeObject[fabricMethod]({ [fabricProperty]: value });
	} else {
		console.error(
			`Method '${fabricMethod}' does not exist on '${fabricObject}' or is not a function.`
		);
	}
}
...
function applyMethod(activeObject, fabricObject, fabricMethod) {
	if (!activeObject[fabricMethod]) {
		console.error(
			`Method '${fabricMethod}' does not exist for Fabric.js object.`
		);
		return;
	}
...
Actions.init({
	...
	endEvent: "fabric",
	callback: (action) => {
		const canvasElements = queryElements({
			element: action.form,
			prefix: "fabric"
		});
